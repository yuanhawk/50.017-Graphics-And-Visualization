...
void calcCylindricalMapping(void)
{
    for (unsigned int i = 0; i < myObject.vertices.size(); i++)
    {
        float x = myObject.vertices[i].v[0];
        float y = myObject.vertices[i].v[1];
        float z = myObject.vertices[i].v[2];
        ...
        myObject.vertices[i].t[0] = r * cos(theta);
        myObject.vertices[i].t[1] = r * sin(theta);
        myObject.vertices[i].t[2] = myObject.vertices[i].v[2] * phi;
    }
}
...
void calcSphereMapping(void)
{
    for (unsigned int i = 0; i < myObject.vertices.size(); i++)
    {
        ...
        auto rho = (float) sqrt(x * x + y * y + z * z);
        ...
        myObject.vertices[i].t[0] = rho * cos(theta) * sin(phi);
        myObject.vertices[i].t[1] = rho * sin(theta) * sin(phi);
        myObject.vertices[i].t[2] = rho * cos(phi);
    }
}
